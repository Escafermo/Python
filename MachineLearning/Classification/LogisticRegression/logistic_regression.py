...
logistic_regressor_classifier = LogisticRegression(random_state = 0)
logistic_regressor_classifier.fit(x_train, y_train)

# PREDICT A NEW RESULT WITH LOGISTIC REGRESSION
y_pred = logistic_regressor_classifier.predict(x_test)
...
plt.contourf(X1, X2, logistic_regressor_classifier.predict(np.array([X1.ravel(), X2.ravel()]).T).reshape(X1.shape),
